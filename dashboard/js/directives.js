...
.directive('button', cardCollapseDirective).directive('svgMap', ['$compile', function ($compile) {
  return {
      restrict: 'A',
      templateUrl: 'img/nigeria.svg',
  ...
  return {
      ...
      link: function (scope, element, attrs) {
          ...
          scope.regionMouseOver = function () {
              ...
              if(scope.dummyData[scope.elementId] == undefined) { c = "Data unavailable"; }else{ c = scope.dummyData[scope.elementId].data; }
              var distr = scope.dummyData[scope.elementId].distributors+" Distributors";
              var retailrs = scope.dummyData[scope.elementId].retailers+" Retailers";
              $description.html($("#"+scope.hoverRegion).attr('title')+"<br/>"+c+"<br/>"+distr+"<br/>"+retailrs);
              $description.css({
                left:  $(document).pageX,
                top:   $(document).pageY - 70
              });
          };
    ...
    return function (input) {
        var b =  Math.floor(input * 255);
        var r = 255 -Math.floor(input * 255);
