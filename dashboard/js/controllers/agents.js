angular
  .module('app')
  .controller('agentCtrl', ['$scope', '$location', '$http', '$state', 'UserService', 'DTOptionsBuilder', '$rootScope', '$timeout',  agentCtrl]);
	function agentCtrl($scope, $location, $http, $state, UserService, DTOptionsBuilder, $rootScope, $timeout) {     
        if($rootScope.validate()){
            ...
            var  url= UserService.apiRoot+'fmcg/get/retailer/-/-';
            $http.get(url, config).then(function(response){
                if(response.data.error.status == 0){
            ...
            $scope.filter = function($obj){
                ...
                var  url= UserService.apiRoot+'fmcg/get/retailer/'+state+'/'+lg;
                config = $rootScope.getHeader(1);
                $http.get(url, config).then(function(response){
                    if(response.data.error.status == 0){
                        $scope.services = response.data.content.data;
            ...
            $scope.viewQ = function(obj, data){
                $scope.lastX = data;
                var  url= UserService.apiRoot+'fmcg/get/transactions/0/ret/'+data.phone+'/0/90988796798979098/'+$('#product').find(":selected").val();
                var data= {};
                config = $rootScope.getHeader(1);
                $http.get(url, config).then(function(response){
                    if(response.data.error.status == 0){
